...
void Ham (Vector s)
{
   ...
   /*printf("hamWinSize:%d frameSize:%d\n",hamWinSize,frameSize);*/
...
void PreEmphasise (Vector s, float k)
{
   ...
   for (i=VectorSize(s);i>=2;i--) {
      s[i] -= s[i-1]*preE;
      /*printf("s[i]:%lf preE:%lf",s[i],preE);*/
   }
   /*printf("\n");*/
   s[1] *= 1.0-preE;
}
                    ...
                    float alpha, float warpLowCut, float warpUpCut)
{
   ...
   lopass = 300.0;
   printf("fb.fres:%lf sampPeriod:%ld fb.fftN:%d\n",fb.fres,sampPeriod,fb.fftN);
   maxChan = numChans+1;
  ...
  /* if (trace&T_MEL){*/
      ...
      printf("Mel passband %f to %f alpha:%lf\n",mlo,mhi,alpha);
  /* }*/
   ...
   for (k=1,chan=1; k<=Nby2; k++){
      ...
      /*printf("melk:%lf,k:%d,fres:%lf,loChan[k]:%d klo:%d,khi:%d Nby2:%d\n",melk,k,fb.fres,fb.loChan[k],fb.khi,fb.klo,Nby2); */
   }
...
void Wave2FBank(Vector s, Vector fbank, float *te, FBankInfo info)
{
   ...
   ZeroVector(fbank);
  /*printf("filter bankinfo.klo:%d,info.khi:%d\n",info.klo,info.khi); */
   for (k = info.klo; k <= info.khi; k++) {             /* fill bins */
      ...
      bin = info.loChan[k];
      t1 = info.loWt[k]*ek;
      /*printf("ek:%lf bin:%d,t1:%f info.usePower:%d\n",ek,bin,t1,info.usePower);*/
      if (bin>0) fbank[bin] += t1;
      if (bin<info.numChans) fbank[bin+1] += ek - t1;
      /*printf("fbank[%d]:%lf ",bin,fbank[bin]);*/
   }
   /* Take logs */
   if (info.takeLogs)
...
void FBank2MFCC(Vector fbank, Vector c, int n)
{
   ...
   /*printf("===FBank2MFCC:n:%d,numChan:%d=====",n,numChan);*/
   for (j=1; j<=n; j++)  {
                    ...
                    int delwin, int head, int tail, Boolean simpleDiffs)
{
   ...
   /*printf("vsize:%d,n:%d,step:%d,offset:%d,delwin:%d,head:%d,tail:%d,simpleDiffs:%d",vSize,n,step,offset,delwin,head,tail,simpleDiffs);*/
   sigmaT2 = 0.0;
   for (t=1;t<=delwin;t++)
      sigmaT2 += t*t;
   sigmaT2 *= 2.0;
      ...
      for (j=1;j<=vSize;j++){
         ...
         if (simpleDiffs)
            *fp2 = (*forw - *back) / (2*delwin);
             ...
             /*while(1);*/
            *fp2 = sum / sigmaT2;
            /* printf("*fp2:%lf sum:%lf sigmaT2:%lf\n",*fp2,sum,sigmaT2);*/
            if(*fp2 == 0.1190)
                while(1);
         }
         ++fp1; ++fp2;
