   ...
   case SHAREDHS:
      ...
      if (se->nMix==1){     /* Single Mixture Case */
         ...
         if (pre==NULL) {
            ...
            bx += det;
           /* printf("pre is null:%lf,det:%lf\n",bx,det);*/
         } else if (pre->id!=id) {
            bx= MOutP(ApplyCompFXForm(me->mpdf,v,inXForm,&det,id),me->mpdf);
            bx += det;
            pre->id=id;
            pre->outp=bx;
    /*        printf("pre->id != id:%lf,det:%lf\n",bx,det);*/
         }
         else {
            bx=pre->outp;
         /*   printf("else bx:%lf\n",bx);*/
...
static void StepHMM1(NetNode *node) /* Model internal propagation NBEST */
{
   ...
   for(i = 1;i<=N;i++) {
       for(j = 1;j<=N;j++)
           printf("%.4lf ",exp(trP[i][j]));
       printf("\n");
   }
   printf("\n");*/
   /*printf("word name:%s\n",node->info.pron->word->wordName->name);*/
   seIndex=pri->psi->seIndexes[hmm->tIdx];
   /*for(i = 2;i<=N;i++) {
       for(j = 0;j<=1;j++) 
           printf("%d ",seIndex[i][j]);
       printf("\n");
   }
   printf("\n");*/

   MLink ml=FindMacroStruct(pri->psi->hset,'h',hmm);
   if(!strcmp(ml->id->name,"sil")){
          isPrint = TRUE; 
   }   
   if(isPrint)
       printf("node name:%s\n numStates:%d",ml->id->name,N);
   for (j=2,res=pri->psi->sBuf+2;j<N;j++,res++) {  /* Emitting states first */
      ...
      if(isPrint)
          printf("cur:%lf trP[%d][%d]:%lf\n",res->tok.like,i,j,trP[i][j]);

      for (i++,cur++;i<=endi;i++,cur++) {
         cmp.tok=cur->tok;
         cmp.tok.like+=trP[i][j];
         if(isPrint)
             printf("cmp:%lf trP[%d][%d]:%lf\n",cmp.tok.like,i,j,trP[i][j]);
   ...
   for (i=1,res=pri->psi->sBuf+1,cur=inst->state;
      ...
      printf("inst->state res->tok.like:%lf\n",res->tok.like);
      for (k=0;k<res->n;k++) cur->set[k]=res->set[k];
   }

   /* Set up pruning limits */
   if(isPrint)
       printf("max.like:%lf",max.like);
   ...
   printf("trp i:%d N:%d endi:%d\n",i,N,endi);
   for (i++,cur++;i<=endi;i++,cur++) {
      cmp.tok=cur->tok; 
      cmp.tok.like+=trP[i][N];
      printf("trp :%lf ",trP[i][N]); 
      if (res->n==0) {
         if (cmp.tok.like > res->tok.like) {
            res->tok=cmp.tok;
         }
   ...
   if (res->tok.like>LSMALL){
      printf("last res->tok.like:%lf\n",res->tok.like); 
      tok.like=res->tok.like+inst->wdlk;
      printf("last 2 res->tok.like:%lf inst->wdlk:%lf\n",res->tok.like,inst->wdlk); 
      if (tok.like > pri->wordMaxTok.like) {
         pri->wordMaxTok=tok;
         pri->wordMaxNode=node;
         /*printf("max node name:%s\n",node->info.pron->word->wordName->name);   */
      }
      if (!node_tr0(node) && pri->models) {
          printf("!node_tr0(node) && pri->models");
...
static void StepHMM2(NetNode *node) 
{
   ...
   printf("StepHMM2 cmp.tok.like:%lf",cmp.tok.like);
   if (res->n==0) {
      if (cmp.tok.like>res->tok.like)
         res->tok=cmp.tok;
   }
     ...
     /* Must be able to survive doing this twice !! */
{
   ...
   if (tok.like>pri->genThresh) {
      for(i=0,dest=node->links;i<node->nlinks;i++,dest++) {
         ...
         printf("tok.like:%lf xtok.tok.like:%lf\n",tok.like,xtok.tok.like);
...
void ProcessObservation(VRecInfo *vri,Observation *obs,int id, AdaptXForm *xform)
{
   ...
   printf("pri->genThresh:%lf pri->genMaxTok.like:%lf vri->genBeam\n",pri->genThresh,vri->genBeam);
   ...
   for (inst=pri->head.link,j=0;inst!=NULL && inst->node!=NULL;inst=next,j++)
      if (inst->max<pri->genThresh) {
         next=inst->link;
         DetachInst(inst->node);
         printf("DetachInst(inst->node);");
      }
      else {
         pri->nxtInst=inst;
         printf("StepInst2(inst->node);\n");
         StepInst2(inst->node);
         next=pri->nxtInst->link;
      }
