...
LogFloat MOutP(Vector x, MixPDF *mp)
{
   ...
   vSize=VectorSize(x);
   switch (mp->ckind) {
   case DIAGC:    px=DOutP(x,vSize,mp); printf("DIAGC");break;
   case INVDIAGC: px=IDOutP(x,vSize,mp);printf("INVDIAGC:%lf\n",px); break;
   case FULLC:    px=FOutP(x,vSize,mp); printf("FULLC");break;
   case LLTC:     px=COutP(x,vSize,mp); printf("LLTC");break;
   case XFORMC:   px=XOutP(x,vSize,mp); printf("XFORMC");break;
   default:       px = LZERO;
   }
   return px;
}
